...
SELECT
    ...
    CASE
        ...
        AND (
            100.0 * T.AttendedHours / T.TotalCourseHours
        ) >= 80 THEN 'Eligible'
        ELSE 'Not Eligible'
    END AS Eligibility
FROM (
        SELECT
            VA.StudentID, VA.StudentName, VA.CourseID, VA.CourseCode, VA.CourseName, IFNULL (SUM(VA.DurationHours), 0) AS TotalCourseHours, IFNULL (
                SUM(
                    CASE
                        WHEN VA.Status IN ('Present', 'Medical') THEN VA.DurationHours
                        ELSE 0
                    END
                ), 0
            ) AS AttendedHours, CASE
                WHEN IFNULL (SUM(VA.DurationHours), 0) > 0 THEN ROUND(
                    100 * SUM(
                        CASE
                            WHEN VA.Status IN ('Present', 'Medical') THEN VA.DurationHours
                            ELSE 0
                        END
                    ) / SUM(VA.DurationHours), 2
                )
                ELSE 0
            END AS AttendancePercentage
        FROM View_Attendance VA
        GROUP BY
            VA.StudentID, VA.StudentName, VA.CourseID, VA.CourseCode, VA.CourseName
    ) AS T
